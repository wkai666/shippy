...
func (repo *ConsignmentRepository) Close() {
    // Close 会在每次结束查询的额时候关闭对话
    // Mgo 会在启动的时候生成一个"主"会话
    // Copy 可以直接从主会话复制出来新会话执行，即每个查询都会有自己的数据库会话
    // 同时，每个会话都有自己连接到数据库的 socket 及错误处理，这么做既安全又高效
    // 如果只使用一个连接到数据库的主 socket 来执行查询，那很多请求处理都会阻塞
    // Mgo 因此能在不使用锁的情况下完美处理并发请求
    // 不过弊端就是，每次结束查询之后，必须确保数据库会话要手动 Close
    // 否则将简历过多无用的连接，浪费数据库资源
    repo.session.Close()
}
